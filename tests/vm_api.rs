...
fn vm_api_can_receive_info() {
    VmBuilder::new().run(|mut vm| async move {
        let info = vm.get_info().await.unwrap();
...
fn vm_api_can_get_firecracker_version() {
    VmBuilder::new().run(|mut vm| async move {
        let firecracker_version = vm.get_firecracker_version().await.unwrap();
...
fn vm_api_can_put_and_get_mmds_untyped() {
    VmBuilder::new().simple_networking().mmds().run(|mut vm| async move {
        vm.create_mmds_untyped(&serde_json::to_value(MmdsData { number: 4 }).unwrap())
            .await
            .unwrap();
        let data = serde_json::from_value::<MmdsData>(vm.get_mmds_untyped().await.unwrap()).unwrap();
...
fn vm_api_can_patch_mmds_untyped() {
    VmBuilder::new().simple_networking().mmds().run(|mut vm| async move {
        vm.create_mmds_untyped(&serde_json::to_value(MmdsData { number: 4 }).unwrap())
            .await
            .unwrap();
        vm.update_mmds_untyped(&serde_json::to_value(MmdsData { number: 5 }).unwrap())
            .await
            .unwrap();
        let data = serde_json::from_value::<MmdsData>(vm.get_mmds_untyped().await.unwrap()).unwrap();
...
fn vm_api_can_put_and_get_mmds_typed() {
    VmBuilder::new().simple_networking().mmds().run(|mut vm| async move {
        vm.create_mmds(MmdsData { number: 4 }).await.unwrap();
        let data = vm.get_mmds::<MmdsData>().await.unwrap();
        assert_eq!(data.number, 4);
        shutdown_test_vm(&mut vm).await;
    });
}
...
fn vm_api_can_patch_mmds_typed() {
    VmBuilder::new().simple_networking().mmds().run(|mut vm| async move {
        vm.create_mmds(MmdsData { number: 4 }).await.unwrap();
        vm.update_mmds(MmdsData { number: 5 }).await.unwrap();
        let data = vm.get_mmds::<MmdsData>().await.unwrap();
        assert_eq!(data.number, 5);
        shutdown_test_vm(&mut vm).await;
    });
